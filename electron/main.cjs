...
const { app, BrowserWindow, Tray, Menu, nativeImage, shell, ipcMain, Notification, nativeTheme } = require("electron");
...
function buildTrayMenu() {
  ...
  if (hint.hasUpdate) {
    items.push({ label: `发现新版本 v${hint.latestVersion} →`, click: () => updater.focusUpdate() });
...
if (!gotLock) {
  ...
  app.whenReady().then(() => {
    ...
    createTray();
    scheduler.start();
    // 软件更新：托盘提示/通知跳转的回调注入，启动后由 updater 自行定时检查
    updater.init({ onTrayRefresh: refreshTrayMenu, onShowWindow: showWindow });
  ...
  app.on("before-quit", (e) => {
    // 已下载完成的更新：拦截本次退出，静默安装后自动重启（installTriggered 防 quitAndInstall 内部再次 quit 造成循环）
