...
/** 应用主题同步到原生窗口框架（标题栏/边框）颜色；主题变化时（含恢复备份后重载）调用 */
function applyNativeTheme(nativeTheme, cfg) {
...
function register(ctx) {
  const { ipcMain, app, shell, nativeTheme } = ctx;
  ...
  ipcMain.handle("load_config", () => {
    ...
    applyNativeTheme(nativeTheme, cfg);
    return cfg;
  });
  ...
  ipcMain.handle("save_config", (_e, args) => {
    try {
      ...
      // 主题切换（顶栏按钮/设置页）保存时同步原生标题栏颜色
      applyNativeTheme(nativeTheme, args.config);
