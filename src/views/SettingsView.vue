...
/** 检查/下载/安装的 IPC 直接返回最新状态快照，直接用返回值更新（事件推送持续补充后续变化） */
async function checkUpdate() {
  try {
    const r = await api.checkUpdate();
    if (r) update.value = r;
  } catch {
    update.value = { ...(update.value as UpdateStatus), status: "error", message: "操作失败，请重试或前往 GitHub 手动下载更新" } as UpdateStatus;
  }
}
...
/** 更新状态行文案：message（防抖提示/错误说明等）优先，其次为各状态的默认文案 */
const updateLine = computed(() => {
  const u = update.value;
  if (!u) return "点击「检查更新」获取最新版本信息";
  if (u.message && ["available", "downloading", "downloaded"].includes(u.status)) return u.message;
  switch (u.status) {
...
// ===== 本机存储（备份压缩包） =====
const storageTab = ref<"webdav" | "local">("webdav");
const backupInfo = ref<BackupInfo | null>(null);
const backupBusy = ref(false); // 恢复进行中（备份走同步引擎，看 app.syncing / app.sync.running）
const backupResult = ref<{ ok: boolean; message: string } | null>(null);
let backupResultTimer = 0;

async function refreshBackupInfo() {
  try {
    backupInfo.value = await api.getBackupInfo();
  } catch {
...
/** 立即备份：走同步引擎强制备份模式，与顶栏「立即备份」（未配 WebDAV 时）等效 */
function backupNow() {
  app.startSync("backup");
}
/** 从压缩包恢复：整包还原数据与设置，成功后重拉配置与页面数据 */
async function restoreBackup() {
  const r = await api.browseBackupFile();
  ...
  try {
    const res = await api.restoreBackup(r.path);
...
const backupButtonDisabled = computed(() => app.syncing || app.sync.running || backupBusy.value);
    ...
    <div class="card">
      <div class="setting-group">
        <div class="sg-title">
          ...
          数据存储
          <span class="sg-hint">WebDAV 多设备同步 · 本机备份压缩包</span>
        </div>
        <div class="tabs" style="margin-bottom: 14px">
          <button class="tab" :class="{ active: storageTab === 'webdav' }" @click="storageTab = 'webdav'">WebDAV 存储</button>
          <button class="tab" :class="{ active: storageTab === 'local' }" @click="storageTab = 'local'">本机存储</button>
        </div>

        <!-- WebDAV 存储：与原「WebDAV 存储」组一致 -->
        <div v-if="storageTab === 'webdav'" class="form-grid">
        ...
        <!-- 本机存储：无 WebDAV 时的数据安全网（备份压缩包 + 整包还原） -->
        <div v-else>
          <div class="switch-row" style="align-items: flex-start">
            <div class="s-left">
              <div class="s-title">备份目录</div>
              <div class="s-desc">备份压缩包（dosage-sync-backup.zip）的存放位置，始终覆盖为最新一份</div>
          ...
          <div class="switch-row">
            ...
            <button class="btn-outline" :disabled="backupButtonDisabled" :title="backupButtonDisabled ? '同步或恢复进行中' : ''" @click="backupNow">{{ app.syncing || app.sync.running ? "进行中" : "立即备份" }}</button>
          </div>
          <div class="switch-row">
            <div class="s-left">
      ...
      <div class="setting-group" style="margin-bottom: 0">
        ...
        <div class="switch-row" id="update-section" style="align-items: flex-start">
          <div class="s-left" style="min-width: 0">
            <div class="s-title">软件更新</div>
            <div class="s-desc" :style="{ ...updateLineStyle, overflowWrap: 'break-word' }">{{ updateLine }}</div>
            <div v-if="update && update.status === 'available' && !update.isPortable && update.notes" class="s-desc" style="white-space: pre-wrap; color: var(--text-2); margin-top: 6px; overflow-wrap: break-word">{{ update.notes }}</div>
