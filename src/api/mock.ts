...
function genRecords(count: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const source = ["zcode", "codex", "dsh", "workbuddy", "reasonix", "codebuddy", "qoder", "qoder-cn"][i % 8];
    // Qoder 官方模型：本地 token 全 0，按 credits 额度点计量（与真实口径一致）
    const official = source === "qoder";
    const input = official ? 0 : Math.round(8000 + seed(i * 13) * 90000);
    const output = official ? 0 : Math.round(300 + seed(i * 7) * 4000);
    ...
    recs.push({
      ...
      deviceName: "这台电脑",
      source,
      providerId: official ? "Qoder" : m.provider,
      modelId: official ? "qmodel-38max" : m.model,
      variant: "max",
        ...
        switch (cmd) {
          case "load_config":
            resolve({
              ...
              sourceVisibility: {
                order: ["zcode", "codex", "dsh", "workbuddy", "reasonix", "codebuddy", "qoder", "qoder-cn"],
                hidden: [],
                initialized: true,
              },
          ...
          case "get_devices": {
            ...
            resolve([
              { deviceId: LOCAL_DEVICE, deviceName: "笔记本", source: "zcode,codex,dsh,workbuddy,reasonix,codebuddy,qoder,qoder-cn", lastSyncAt: Date.now() - 60000, online: true, totalTokens: local, isLocal: true, cost: allRecords.reduce((n, r) => n + calcCost(r), 0) },
            ]);
            break;
          }
