    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        switch (cmd) {
          ...
          // ===== 软件更新（浏览器演示环境：固定返回可预览 UI 各状态的假数据） =====
          ...
          case "download_update":
            resolve({
              status: "downloaded", isPortable: false, currentVersion: "1.0.0", latestVersion: "1.8.0",
              percent: 100, notes: "演示环境：新增应用内自动更新（安装版）", message: "",
            });
            break;
          case "install_update":
            resolve({
