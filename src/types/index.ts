...
export interface UsageRecord {
  ...
  source: string; // 软件源："zcode" | "codex" | "dsh" | "workbuddy" | "reasonix" | "codebuddy" | "qoder" | "qoder-cn" | ...
  ...
  credits?: number; // 额度点（Qoder 官方模型按 credits 计量、token 为 0；其余源无此字段）
...
export interface AppConfig {
  ...
  update: UpdateConfig; // 软件更新设置
}

/** 软件更新设置（仅「自动检测新版本」开关；检测频率为主进程代码内常量） */
