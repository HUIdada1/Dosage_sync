...
- **计费（2026-09 新增）**：按「模型单价 × token 分解」动态核算费用——价格带时段版本（改价自动生成新版本，历史费用按记录发生时刻的价格重算），支持手动维护、从用量一键生成草稿、LiteLLM/OpenRouter 价格源导入预览；价格表经 WebDAV 多设备共享（LWW）。内置 20 个常用模型的默认价格，开启计费即见全部历史费用。侧栏「费用」页展示费用 KPI/趋势/三维度费用榜，「计费规则」页管理价格（默认关闭，在计费规则页开启）。
